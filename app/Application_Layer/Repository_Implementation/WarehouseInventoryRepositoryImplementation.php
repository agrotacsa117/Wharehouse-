...
class WarehouseInventoryRepositoryImplementation implements WarehouseInventoryRepositoryInterface
{
    ...
    {
        return WarehouseInventoryModel::query()
            ->select(
                DB::raw(
                    'SUM(
                        CASE WHEN DATEDIFF(
                        expiration_date, 
                        CURDATE()) < 0 THEN quantity 
                ...
                DB::raw(
                    'SUM(
                        CASE WHEN DATEDIFF(
                            expiration_date,
                            CURDATE()) BETWEEN 90 
                            AND 120 THEN quantity 
                            ELSE 0 END) as expired_soon'),
                DB::raw(
                    'SUM(
                        CASE WHEN DATEDIFF(
                        expiration_date, 
                        CURDATE()) BETWEEN 0 
                        AND 90 THEN quantity 
                        ELSE 0 END) as endangered')
            )
            ->where('warehouse_id', $warehouseId)
            ->where('product_id', $productId)
            ->first()->toArray();
