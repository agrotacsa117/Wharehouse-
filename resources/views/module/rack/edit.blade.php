...
<main id="main" class="main bg-light py-4">
    <div class="pagetitle d-flex justify-content-between align-items-center mb-4">
      <h1 class="fw-bold text-primary"><i class="fa-solid fa-tags me-2"></i>Editar Rack o Aduana</h1>
        ...
        <div class="col-lg-7 col-xl-6">
          <div class="card shadow border-0">
            <div class="card-header bg-primary text-white">
              <h5 class="card-title mb-0"><i class="fa-solid fa-tag me-2"></i>Formulario de Edición de Rack o Aduana</h5>
            </div>
              ...
              <form action="{{ route('rack.update', $rack->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('rack_aduana') is-invalid @enderror" 
                           id="rack_aduana" name="rack_aduana" value="{{ old('rack_aduana', $rack->rack_aduana) }}" required placeholder="Ej: RACK-01">
                    @error('rack_aduana')
                ...
                <div class="mb-4">
                    ...
                    <input type="number" class="form-control @error('cantidad_max') is-invalid @enderror" 
                           id="cantidad_max" name="cantidad_max" value="{{ old('cantidad_max', $rack->cantidad_max) }}" required placeholder="Ej: 100">
                    @error('cantidad_max')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-select @error('bodega') is-invalid @enderror" id="bodega" name="bodega" required>
                        ...
                        @foreach($bodegas as $key => $nombre)
                            <option value="{{ $key }}" {{ old('bodega', $rack->bodega) == $key ? 'selected' : '' }}>{{ $nombre }}</option>
                        @endforeach
                    </select>
                    @error('bodega')
                        <div class="invalid-feedback">{{ $message }}</div>
